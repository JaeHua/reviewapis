...
func (m *CreateReviewRequest) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetContent()); l < 8 || l > 512 {
		err := CreateReviewRequestValidationError{
			field:  "Content",
			reason: "value length must be between 8 and 512 runes, inclusive",
		}
		if !all {
			return err
		}
...
func (m *AuditReviewRequest) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetOpReason()); l < 2 || l > 256 {
		err := AuditReviewRequestValidationError{
			field:  "OpReason",
...
func (m *AppealReviewRequest) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetReason()); l < 1 || l > 256 {
		err := AppealReviewRequestValidationError{
			field:  "Reason",
